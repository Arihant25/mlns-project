"""
train_weighted_attentivefp.py
=============================
Phase 3C extension - Soft-weighted training for an AttentiveFP-style model.

Trains AttentiveFPSimModel on 100% of training data while down-weighting
per-sample squared error using aleatoric uncertainty from EvidentialGSLModel:
...
import numpy as np
import torch
from model_attentivefp import AttentiveFPSimModel
...
def run_single_seed(seed, gamma, train_ds, val_ds, test_ds, scaler):
    ...
    model = AttentiveFPSimModel().to(DEVICE)
...
def main():
    ...
    lines = [
        "Phase 3C - Soft-Weighting Report (AttentiveFP-style, Multi-Seed)",
        "===============================================================",
    ...
    report_path = os.path.join(
        RESULTS_DIR, "phase3c_soft_weighting_attentivefp_report.txt"
    )
