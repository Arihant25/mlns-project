...
def run_single_seed(
    seed, suffix, label, train_emb, train_tgt, val_emb, val_tgt, test_emb, test_tgt
):
    ...
    print(
        f"    [seed={seed}] Test → RMSE={rmse:.4f}  MAE={mae:.4f}  "
...
def main():
    ...
    for suffix, label in zip(SUFFIXES, LABELS):
        ...
        for seed in SEEDS:
            ...
            done = len(seed_metrics)
            avg_seed_s = (time.perf_counter() - run_start) / done
            eta_s = avg_seed_s * (len(SEEDS) - done)
            print(f"    [run ETA] remaining: {format_eta(eta_s)}")
